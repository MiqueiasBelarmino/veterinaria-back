  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    for (const scope of requiredScopes) {
      ...
      if (scope.endsWith(':own')) {
        ...
        const id = req.params?.id ?? req.params?.[`${resource.slice(0, -1)}Id`] ?? req.params?.petId ?? req.body?.id ?? req.body?.appointmentId ?? req.body?.petId;
        if (!id) continue;
        ...
        if (resource === 'appointments') {
          ...
          // Client owner
          if (user.role === 'CLIENT' && appt.pet?.client?.userId === user.id) return true;

          // Vet ownership or clinic membership
          if (user.role === 'VET') {
            const vet = await this.prisma.vet.findUnique({ where: { userId: user.id } });
            if (!vet) continue;
            if (vet.id === appt.vetId) return true;
            if (vet.clinicId && appt.vet && appt.vet.clinicId && vet.clinicId === appt.vet.clinicId)
              return true;
          }
        }
        ...
        if (resource === 'plans') {
          ...
          if (!plan) continue;
          if (user.role === 'CLIENT' && plan.pet?.client?.userId === user.id) return true;
          if (user.role === 'VET') {
            const vet = await this.prisma.vet.findUnique({ where: { userId: user.id } });
            if (!vet) continue;
            // vets of same clinic can manage
            const planVet = await this.prisma.vet.findUnique({ where: { id: plan?.pet?.id } }).catch(() => null);
            if (vet.clinicId && planVet && planVet.clinicId && vet.clinicId === planVet.clinicId) return true;
        ...
        if (resource === 'prescriptions') {
          ...
          if (!presc) continue;
          if (user.role === 'CLIENT' && presc.pet?.client?.userId === user.id) return true;
          if (user.role === 'VET') {
            const vet = await this.prisma.vet.findUnique({ where: { userId: user.id } });
            if (!vet) continue;
            if (vet.id === presc.vetId) return true;
            if (vet.clinicId && presc.vet && presc.vet.clinicId && vet.clinicId === presc.vet.clinicId) return true;
          }
        }
        ...
        if (resource === 'clinical-records') {
          const record = await this.prisma.clinicalRecord.findUnique({
            where: { id },
            include: { pet: { include: { client: true } }, appointment: { include: { vet: true } } },
          });
          if (!record) continue;
          if (user.role === 'CLIENT' && record.pet?.client?.userId === user.id) return true;
          if (user.role === 'VET') {
            const vet = await this.prisma.vet.findUnique({ where: { userId: user.id } });
            if (!vet) continue;
            if (record.appointment?.vetId === vet.id) return true;
            if (vet.clinicId && record.appointment?.vet && record.appointment.vet.clinicId && vet.clinicId === record.appointment.vet.clinicId) return true;
          }
        }
        if (resource === 'educational-materials') {
          const material = await this.prisma.educationalMaterial.findUnique({
            where: { id },
            include: { pets: { include: { client: true } }, appointments: { include: { pet: { include: { client: true } }, vet: true } } },
          });
          if (!material) continue;
          // If any linked pet belongs to client, allow
          if (user.role === 'CLIENT') {
            const belongs = (material.pets || []).some(p => p.client?.userId === user.id);
            if (belongs) return true;
            // check appointments' pet owners as well
            const appBelongs = (material.appointments || []).some(a => a.pet?.client?.userId === user.id);
          ...
          if (user.role === 'VET') {
            ...
            const appVetMatch = (material.appointments || []).some(a => a.vet && a.vet.clinicId && vet.clinicId && a.vet.clinicId === vet.clinicId);
